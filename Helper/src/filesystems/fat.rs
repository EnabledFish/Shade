...
pub fn fat_create_image_from_folder(
    from_folder: &PathBuf,
    to_file: &PathBuf,
    sector_size: usize,
    sector_count: usize,
    ...
    let mut disk_image = vec![0u8; sector_size * sector_count];

    {
        ...
        partition_byte_range = mbr_get_partition_byte_range(&mbr, sector_size);
        ...
        fatfs::format_volume(
            &mut cursor,
            FormatVolumeOptions::new().volume_label(*b"ShadeSystem"),
        ).unwrap();
...
fn mbr_get_partition_byte_range(mbr: &MBR, sector_size: usize) -> Range<usize> {
